...
router.get('/:id/comments', asyncHandler (async (req, res, next) => {
    const data = await MainModel.listItems({id: req.params.id, 'query': req.query}, {task: 'getComments'})
    if(!data) return res.status(200).json({success : true,data : "Dữ liệu rỗng"})
    res.status(201).json({
        success: true,
        count: data.length,
...
router.get('/:id/posts-images', asyncHandler (async (req, res, next) => {
    const data = await MainModel.listItems({id: req.params.id, 'query': req.query}, {task: 'getPostsImages'})
    if(!data) return res.status(200).json({success : true,data : "Dữ liệu rỗng"})
    res.status(201).json({
        success: true,
        count: data.length,
